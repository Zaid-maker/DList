...
export class AddGuildComponent implements AfterViewInit {
  ...
  private initFormValue() {
    for (const key in this.savedGuild.listing)
      this.form.controls[key]?.setValue(this.savedGuild.listing[key]);
  }
  ...
  filterTags(filter: string): void {
    this.filteredTags = this.tagService.tags.filter((tag) =>
      tag.name.toLowerCase().includes(filter.toLowerCase())
    );
  }

  async update() {
    if (this.form.invalid) return this.form.setErrors({ invalid: true });
